...
CREATE TABLE orders (
    ...
    PRIMARY KEY (id),
    KEY idx_orders_account_status_created (account_id, status, dt_created),
    KEY idx_orders_active (account_id, dt_deleted)
...
| Soft deletes | `dt_deleted DATETIME NULL` plus scoped indexes | Filtering soft-deleted rows without an index |
...
CREATE INDEX idx_orders_account_status_created
    ON orders (account_id, status, dt_created);
...
```sql
SELECT id, name, dt_created
FROM products
WHERE (dt_created, id) < (?, ?)
ORDER BY dt_created DESC, id DESC
LIMIT 50;
```
...
```sql
CREATE INDEX idx_products_created_id ON products (dt_created, id);
```
